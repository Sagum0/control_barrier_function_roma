...
TRAIN_PI0_SCRIPT = WORKSPACE_ROOT / "scripts" / "training" / "pi0" / "train.py"

# 인자를 생략했을 때 읽는 한국어 주석 config 경로다.
DEFAULT_CONFIG_PATH = WORKSPACE_ROOT / "config" / "training" / "pi0" / "piper_lora.yaml"
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "config/training/pi0/piper_lora.yaml을 검사한 뒤 기존 검증된 scripts/training/pi0/train.py를 실행합니다."
        )
    )
    parser.add_argument(
        "--config",
...
def build_forward_arguments(
    ...
    *,
    ...
    """YAML 값을 기존 scripts/training/pi0/train.py가 이미 검증하는 CLI 인자로 변환한다."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    from piper_vla.training.pi0.settings import load_pi0_settings
    ...
    # 기존 scripts/training/pi0/train.py가 JAX를 import하기 전에 읽도록 환경 값을 먼저 고정한다.
