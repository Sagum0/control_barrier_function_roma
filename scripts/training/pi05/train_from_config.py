#!/usr/bin/env python3
"""한국어 주석 YAML을 기존 검증된 Piper π0.5 학습 CLI로 변환한다."""
...
TRAIN_PI05_SCRIPT = WORKSPACE_ROOT / "scripts" / "training" / "pi05" / "train.py"

# 인자를 생략했을 때 읽는 한국어 주석 config 경로다.
DEFAULT_CONFIG_PATH = WORKSPACE_ROOT / "config" / "training" / "pi05" / "piper_lora.yaml"
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "config/training/pi05/piper_lora.yaml을 검사한 뒤 기존 검증된 scripts/training/pi05/train.py를 실행합니다."
        )
    )
    parser.add_argument(
        "--config",
...
def build_forward_arguments(
    ...
    *,
    ...
    """YAML 값을 기존 scripts/training/pi05/train.py가 이미 검증하는 CLI 인자로 변환한다."""
...
def main(argv: Sequence[str] | None = None) -> int:
    """YAML을 strict-load하고 기존 검증된 π0.5 학습 process로 교체한다."""
    ...
    from piper_vla.training.pi05.settings import load_pi05_settings

    parser = build_argument_parser()
    namespace = parser.parse_args(arguments)
    settings = load_pi05_settings(namespace.config, WORKSPACE_ROOT)
    ...
    # 기존 scripts/training/pi05/train.py가 JAX를 import하기 전에 읽도록 환경 값을 먼저 고정한다.
    ...
    print("Delegating to     :", TRAIN_PI05_SCRIPT, flush=True)
    os.execv(
        str(WORKSPACE_PYTHON),
        [str(WORKSPACE_PYTHON), str(TRAIN_PI05_SCRIPT), *forwarded],
    )
    raise AssertionError("os.execv가 반환됐습니다.")
