#!/usr/bin/env python3
"""YAML 하나로 기존 vla_pipeline Piper async client를 실행한다."""
...
# 기존 robot/ROS bridge 구현은 vla_pipeline의 검증된 client를 그대로 재사용한다.
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="vla_ws YAML 기반 기존 vla_pipeline Piper async client"
    )
...
def build_client_arguments(settings: object, *, checkpoint_step: int) -> list[str]:
    """draccus가 받는 기존 piper_bridge.async_client argument를 만든다."""

    async_client = getattr(settings, "async_client", None)
    if async_client is None:
        raise ValueError("vla_pipeline client config에는 async_client 설정이 필요합니다.")
    ...
    return [
        ...
        f"--server_address={_server_address(server.host, server.port)}",
        "--robot.type=piper_bridge",
        "--policy_type=pi0",
        f"--pretrained_name_or_path={checkpoint.run_name}/{checkpoint_step}",
        f"--task={policy.prompt}",
        "--policy_device=cuda",
        "--client_device=cpu",
        f"--actions_per_chunk={async_client.actions_per_chunk}",
        f"--chunk_size_threshold={async_client.chunk_size_threshold}",
        f"--aggregate_fn_name={async_client.aggregate_fn_name}",
        f"--fps={async_client.fps}",
        "--debug_visualize_queue_size="
        f"{str(async_client.debug_visualize_queue_size).lower()}",
    ]
...
def build_client_environment(
    ...
    environment["PIPER_EPISODE_TIME_S"] = (
        "" if episode_time is None else f"{episode_time:g}"
    )
...
def _print_resolved_configuration(
    ...
    *,
    ...
    async_client = getattr(settings, "async_client")
...
def main(argv: Sequence[str] | None = None) -> int:
    """checkpoint를 검증한 뒤 기존 async client process로 교체한다."""
    ...
    if not (VLA_PIPELINE_ROOT / "piper_bridge" / "async_client.py").is_file():
        parser.error("기존 piper_bridge.async_client를 찾지 못했습니다.")

    settings = load_pi0_inference_settings(namespace.config, WORKSPACE_ROOT)
    if settings.async_client is None:
        parser.error("vla_pipeline client config에는 async_client 설정이 필요합니다.")
