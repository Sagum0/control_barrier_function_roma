...
DEFAULT_CONFIG_PATH = (
    WORKSPACE_ROOT / "config" / "inference" / "pi0" / "piper_vla_pipeline.yaml"
)

# π0.5 wrapper가 같은 구현을 재사용할 때 바꾸는 client handshake 값이다.
POLICY_TYPE = "pi0"
...
def build_client_arguments(settings: object, *, checkpoint_step: int) -> list[str]:
    ...
    arguments = [
        str(CLIENT_PYTHON),
        "-m",
        "piper_vla.inference.transports.vla_pipeline.sync_client",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if POLICY_TYPE == "pi0":
        from piper_vla.inference.pi0.checkpoint import select_checkpoint
        from piper_vla.inference.pi0.settings import load_pi0_inference_settings as load_settings
    elif POLICY_TYPE == "pi05":
        from piper_vla.inference.pi05.checkpoint import select_checkpoint
        from piper_vla.inference.pi05.settings import load_pi05_inference_settings as load_settings
    else:
        raise ValueError(f"지원하지 않는 POLICY_TYPE입니다: {POLICY_TYPE!r}")
    ...
    settings = load_settings(namespace.config, WORKSPACE_ROOT)
