...
실행에 쓰는 값은 [`piper_lora.yaml`](../../../config/training/pi0/piper_lora.yaml)에 있고, 각 값의 뜻과 조정 방법은 이 문서에 있다.
...
```bash
./scripts/training/pi0/plot_training.py \
...
```bash
./scripts/training/pi0/train_from_config.py \
  --config config/training/pi0/piper_lora.yaml \
  --print-config
```
...
./scripts/training/pi0/prepare_norm_stats.py \
  --config config/training/pi0/piper_lora.yaml
```
