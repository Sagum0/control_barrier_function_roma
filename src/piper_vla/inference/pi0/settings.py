...
from piper_vla.inference.transports.vla_pipeline.async_client_settings import (
...
def load_inference_settings(
    ...
    *,
    ...
    """모델 family의 namespace를 고정해 추론 YAML을 typed 설정으로 변환한다."""
    ...
    if config_name != expected_config_name:
        raise ValueError(
            f"현재 추론기는 Piper {model_label} 학습 namespace만 지원합니다: "
            f"expected={expected_config_name!r}, actual={config_name!r}"
        )
...
def load_pi0_inference_settings(
    ...
    """Piper π0 namespace로 고정해 추론 YAML을 읽는다."""

    return load_inference_settings(
        config_path,
        workspace_root,
        expected_config_name=PIPER_PI0_CONFIG_NAME,
        model_label="π0",
    )
