...
from piper_vla.inference.pi0.checkpoint import SelectedCheckpoint
...
from piper_vla.inference.pi0.settings import Pi0InferenceSettings
...
def require_jax_gpu_backend(
    ...
    if backend != "gpu":
        raise RuntimeError(
            "Piper OpenPI 추론에는 JAX GPU backend가 필요합니다: "
            f"actual={backend!r}"
        )
    return backend
...
def build_inference_train_config(
    ...
    from piper_vla.training.pi0.data import PiperDataConfigFactory
