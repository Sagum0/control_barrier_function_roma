...
from piper_vla.inference.common.observation import (
    build_canonical_observation,
    validate_policy_output,
)
from piper_vla.inference.common.romalab_contract import ACTION_HORIZON, ROBOT_DIM
...
def validate_remote_policy_config(
    ...
    *,
    ...
    expected_policy_type: str = "pi0",
) -> None:
    """client 요청이 고정 OpenPI 서버와 호환되는지 모델 로드 없이 검사한다."""

    if config.policy_type != expected_policy_type:
        raise ValueError(
            f"policy_type은 {expected_policy_type}여야 합니다: "
...
class LeRobotGrpcPolicyService:
    ...
    def __init__(
        ...
        *,
        ...
        if expected_policy_type not in {"pi0", "pi05"}:
            raise ValueError(f"지원하지 않는 policy_type입니다: {expected_policy_type!r}")
        self._policy = policy
        self._expected_actions_per_chunk = expected_actions_per_chunk
        self._expected_policy_type = expected_policy_type
    ...
    def SendPolicyInstructions(self, request: Any, context: Any) -> Any:  # noqa: N802
        ...
        try:
            ...
            validate_remote_policy_config(
                ...
                expected_policy_type=self._expected_policy_type,
            )
    ...
    def GetActions(self, request: Any, context: Any) -> Any:  # noqa: N802, ARG002
        ...
        try:
            ...
            logging.exception("OpenPI inference failed")
            context.abort(
                self._grpc_status("INTERNAL"),
                f"OpenPI inference 실패: {error}",
            )
            raise AssertionError("gRPC context.abort가 반환됐습니다.") from error
