"""Piper π0.5 LoRA YAML을 GPU 초기화 없이 엄격하게 읽는다."""
...
# checkpoint·dataset·추론이 공유하는 현재 π0.5 모델 계약이다.
LOCKED_MODEL_CONTRACT = {
    "openpi_profile": "experimental_pi05_piper_lora",
    "pi05": True,
...
class Pi05PathSettings:
    ...
    # fresh 학습용 π0.5 base params 절대 경로다.
    base_params: Path


@dataclass(frozen=True)
class Pi05DatasetSettings:
...
@dataclass(frozen=True)
class Pi05Settings:
    """검증된 π0.5 YAML의 안전한 실행 설정을 하나로 묶는다."""
    ...
    paths: Pi05PathSettings

    # Dataset/normalization asset 식별 설정이다.
    dataset: Pi05DatasetSettings

    # Parameter 학습 범위를 정하는 fine-tuning 설정이다.
    finetuning: Pi05FinetuningSettings

    # 기존 train_pi0 CLI에 전달할 학습 설정이다.
    training: Pi05TrainingSettings

    # JAX import 전에 적용할 runtime 설정이다.
    runtime: Pi05RuntimeSettings
...
def load_pi05_settings(config_path: Path, workspace_root: Path) -> Pi05Settings:
    """YAML을 읽어 strict schema와 잠긴 π0.5 계약을 검증한다."""
    ...
    if not resolved_config.is_file():
        raise FileNotFoundError(f"π0.5 YAML config가 없습니다: {resolved_config}")
    ...
    return Pi05Settings(
        ...
        dataset=Pi05DatasetSettings(asset_id=asset_id),
        finetuning=Pi05FinetuningSettings(vision_encoder=vision_encoder),
        training=Pi05TrainingSettings(
