"""Piper LeRobot v3 데이터를 OpenPI π0.5 학습 입력으로 연결한다."""
...
# π0.5가 한 번에 예측하는 action chunk 길이다.
PI05_ACTION_HORIZON = 50

# π0.5 내부 state/action 표현의 고정 차원이다.
PI05_MODEL_ACTION_DIM = 32

# π0.5 image encoder에 들어가는 정사각 영상 크기다.
PI05_IMAGE_SIZE = 224

# π0.5 tokenizer가 사용하는 최대 prompt token 길이다.
...
class PiperPi05DataConfigFactory(training_config.DataConfigFactory):
    """Piper v3 canonical sample을 π0.5 학습·추론 transform으로 연결한다."""
    ...
    def create(
        ...
        if model_config.model_type is not model_api.ModelType.PI05:
            raise ValueError(f"현재 데이터 설정은 π0.5만 지원합니다: {model_config.model_type=}")
        if model_config.action_horizon != PI05_ACTION_HORIZON:
            raise ValueError(
                "Piper adapter와 모델의 action horizon이 다릅니다: "
                f"adapter={PI05_ACTION_HORIZON}, model={model_config.action_horizon}"
            )
        if model_config.action_dim != PI05_MODEL_ACTION_DIM:
            raise ValueError(
                "π0.5 model action dimension이 예상값과 다릅니다: "
                f"expected={PI05_MODEL_ACTION_DIM}, actual={model_config.action_dim}"
            )
...
def build_training_data_bundle(
    ...
    *,
    ...
    if not data_config.use_quantile_norm:
        raise ValueError("π0.5는 q01/q99 quantile normalization을 사용해야 합니다.")
...
def validate_training_batch(
    ...
    """실제 JAX batch가 검증된 π0.5 shape, dtype, mask 계약을 만족하는지 확인한다."""

    observation, actions = batch
    batch_size = config.batch_size
    expected_image_shape = (batch_size, PI05_IMAGE_SIZE, PI05_IMAGE_SIZE, 3)
    ...
    if set(observation.images) != set(model_api.IMAGE_KEYS):
        raise ValueError(f"π0.5 camera key가 다릅니다: {tuple(observation.images)}")
    if set(observation.image_masks) != set(model_api.IMAGE_KEYS):
        raise ValueError(f"π0.5 camera mask key가 다릅니다: {tuple(observation.image_masks)}")
    ...
    if observation.state.shape != (batch_size, PI05_MODEL_ACTION_DIM):
    ...
    if actions.shape != (batch_size, PI05_ACTION_HORIZON, PI05_MODEL_ACTION_DIM):
    ...
    if observation.tokenized_prompt.shape != (batch_size, PI05_PROMPT_TOKEN_LENGTH):
        raise ValueError(f"prompt token shape 오류: {observation.tokenized_prompt.shape}")
    if observation.tokenized_prompt_mask.shape != (batch_size, PI05_PROMPT_TOKEN_LENGTH):
