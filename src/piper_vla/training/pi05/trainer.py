"""검증된 OpenPI π0.5 LoRA 학습을 Piper LeRobot v3 데이터에 실행한다."""
...
from piper_vla.training.pi05.data import (
    PI05_ACTION_HORIZON,
    PI05_MODEL_ACTION_DIM,
    PI05_PROMPT_TOKEN_LENGTH,
    PiperPi05DataConfigFactory,
    TrainingBatch,
    TrainingDataBundle,
...
from piper_vla.training.pi05.settings import (
...
# Piper π0.5 checkpoint와 normalization asset을 묶는 설정 이름이다.
PIPER_PI05_CONFIG_NAME = "pi05_piper_lora"

# 검증된 π0.5 language/image expert의 LoRA variant 이름이다.
EXPECTED_PALIGEMMA_VARIANT = "gemma_2b_lora"

# 검증된 π0.5 action expert의 LoRA variant 이름이다.
...
class TrainingOptions:
    """CLI에서 받은 π0.5 학습 경로와 안전 설정을 보관한다."""
    ...
    # fresh 학습에서 불러올 공식 π0.5 base params 경로다.
    base_params: Path
...
def validate_training_options(options: TrainingOptions) -> None:
    ...
    checkpoint_dir = (
        options.runs_root / PIPER_PI05_CONFIG_NAME / options.run_name
    ).resolve()
    if not options.check_only:
        if options.resume:
            ...
            if not options.base_params.is_dir():
                raise FileNotFoundError(f"π0.5 base params 경로가 없습니다: {options.base_params}")
...
def build_pi05_train_config(options: TrainingOptions) -> training_config.TrainConfig:
    """노트북에서 검증한 실험용 Piper π0.5 LoRA 설정을 생성한다."""

    model_config = pi0_config.Pi0Config(
        pi05=True,
        paligemma_variant=EXPECTED_PALIGEMMA_VARIANT,
        action_expert_variant=EXPECTED_ACTION_EXPERT_VARIANT,
    ...
    config = training_config.TrainConfig(
        ...
        ema_decay=None,
        freeze_filter=build_freeze_filter(
            model_config.get_freeze_filter(),
    ...
    validate_pi05_train_config(config, options)
    return config


def validate_pi05_train_config(
    config: training_config.TrainConfig,
    options: TrainingOptions,
) -> None:
    """공식 π0.5 LoRA 구조와 Piper normalization 계약이 보존됐는지 확인한다."""

    if config.name != PIPER_PI05_CONFIG_NAME or config.exp_name != options.run_name:
        raise ValueError("TrainConfig의 config/run 이름이 요청과 다릅니다.")
    if config.model.model_type is not model_api.ModelType.PI05:
        raise ValueError(f"π0.5가 아닌 model type입니다: {config.model.model_type}")
    if not getattr(config.model, "pi05", False):
        raise ValueError("현재 학습 코드는 π0가 아니라 π0.5 전용입니다.")
    if not getattr(config.model, "discrete_state_input", False):
        raise ValueError("π0.5 discrete-state 입력이 꺼져 있습니다.")
    ...
    if config.model.action_dim != PI05_MODEL_ACTION_DIM:
        raise ValueError(f"π0.5 action dimension 오류: {config.model.action_dim}")
    if config.model.action_horizon != PI05_ACTION_HORIZON:
        raise ValueError(f"π0.5 action horizon 오류: {config.model.action_horizon}")
    if config.model.max_token_len != PI05_PROMPT_TOKEN_LENGTH:
        raise ValueError(f"π0.5 prompt token length 오류: {config.model.max_token_len}")
    if config.ema_decay is not None:
        raise ValueError("현재 실험용 π0.5 LoRA baseline은 EMA를 사용하지 않습니다.")
    if config.freeze_filter is None:
        raise ValueError("π0.5 LoRA freeze filter가 없습니다.")
    if config.overwrite:
    ...
    expected_config_root = (options.runs_root / PIPER_PI05_CONFIG_NAME).resolve()
...
def initialize_fresh_runtime(
    ...
    *,
    ...
    """π0.5 base params에서 실제 LoRA TrainState를 만든 뒤 새 run manager를 생성한다."""
    ...
    print("Loading π0.5 base weights and initializing LoRA TrainState...")
...
def execute(options: TrainingOptions) -> int:
    ...
    config = build_pi05_train_config(options)
    ...
    from piper_vla.training.pi05.diagnostics import run_training_loop_with_diagnostics
...
def build_argument_parser() -> argparse.ArgumentParser:
    """자동 overwrite 옵션이 없는 안전한 π0.5 학습 CLI parser를 만든다."""

    parser = argparse.ArgumentParser(
        description="Piper LeRobot v3 데이터로 OpenPI π0.5 LoRA를 학습합니다.",
    )
    parser.add_argument(
        "--run-name",
        required=True,
    ...
    parser.add_argument(
        "--base-params",
        type=Path,
        default=Path("data/cache/openpi/openpi-assets/checkpoints/pi05_base/params"),
        help="fresh 학습에서만 사용하는 π0.5 base params 경로",
    )
    parser.add_argument(
        "--vision-encoder",
...
def main(
    ...
    *,
    ...
    """CLI 인자를 해석해 Piper π0.5 LoRA 학습을 실행한다."""
